...
void UTestMultiplayGameInstance::Init()
{
	...
	if (UEngine* Engine = GetEngine())
	{
		...
		// 서버 주소가 잘못됐다. 서버 인원이 꽉 찼다. 맵 파일이 없다.
...
void UTestMultiplayGameInstance::CreateServer() 
{
	...
	{
		MapName = UGameplayStatics::GetCurrentLevelName(GetWorld());	// 거의 의미 없음.
	}
	...
	UE_LOG(LogTemp, Log, TEXT("리슨서버 생성 : %s, 최대 인원 : %d"), *MapName, MaxPlayers);
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(1, 5.0f, FColor::Cyan, TEXT("리슨서버 시작"));
	}
